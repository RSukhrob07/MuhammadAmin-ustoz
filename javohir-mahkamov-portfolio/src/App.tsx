    ...
    <div id="portfolio-root" className="min-h-screen bg-[#0B0A0F] text-neutral-100 font-sans selection:bg-[#FF6B00]/30 selection:text-white relative overflow-x-hidden antialiased">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16 flex flex-col items-center">
        ...
        <div id="main-portfolio-grid" className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10 items-stretch">
            ...
            <div className="relative group w-full h-[400px] sm:h-[480px] lg:h-[580px] rounded-2xl overflow-hidden border border-neutral-800 bg-[#121118]/80 shadow-[0_0_40px_-15px_rgba(255,107,0,0.25)] flex items-center justify-center transition-all duration-500 hover:border-[#FF6B00]/40">
              ...
              {teacherData.teacherImg && teacherData.teacherImg.trim() !== "" ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-[#1A1625] via-[#0F0E15] to-[#251A12] flex flex-col items-center justify-center gap-5 select-none relative">
                  ...
                  {/* Har doim yorqin ko'rinib turuvchi status badge */}
                  <div className="flex flex-col items-center gap-2 z-10 text-center px-4 mt-1">
                    <span className="px-4 py-1 text-xs font-black uppercase bg-red-600 text-white rounded-md tracking-wider shadow-[0_4px_12px_rgba(220,38,38,0.4)]">
                      IELTS 8.0
                    </span>
                    <span className="text-[10px] text-neutral-400 font-bold tracking-[2px] uppercase mt-0.5">
                      TOP TEACHER
                    </span>
                  </div>
          ...
          {/* O'RTA USTUN: PORTFOLIO KARTALARI (SKRINSHODDAGI DARAXA SHU YERDA AVTOMATIK GRUPOVKA BO'LADI) */}
          <div className="lg:col-span-5 flex flex-col gap-4">
            {teacherData.portfolioCards.map((card, idx) => {
              // Agar sarlavhada "Daraja" so'zi bo'lsa, JSON'da nima yozilganidan qat'iy nazar IELTS 8.0 ga o'zgartiramiz
              const isLevelCard = card.title.toUpperCase().includes("DARAJA");
              const displayTitle = isLevelCard ? "USTOZINGIZNING DARAJASI" : card.title;
              const displayBullet = isLevelCard ? "IELTS 8.0 ustoz" : card.bullet;
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm md:text-base text-neutral-200 leading-relaxed font-normal pl-3 relative flex items-start gap-1.5">
        ...
        {/* FAQAT INSTAGRAM LINK QOLDIRILDI, RO'YXATDAN O'TISH TO'LIQ OLIB TASHLANDI */}
